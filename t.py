...
output_folder = 'Posts_ko'  # 输出的 Markdown 文件夹，指定为德文
...
for filename in os.listdir(input_folder):
    if filename.endswith('.md'):
        ...
        for line in content[4:]:  # 从第五行开始翻译
            ...
            try:
                translated_line = GoogleTranslator(source='auto', target='korean').translate(trimmed_line)
        ...
        for i, line in enumerate(final_content):
            if line.startswith('title ='):
                ...
                new_title = GoogleTranslator(source='auto', target='korean').translate(original_title)  # 翻译标题
