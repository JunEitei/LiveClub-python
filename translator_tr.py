...
output_folder = 'Posts_tr'  # 输出的 Markdown 文件夹，指定为葡萄牙文
...
for filename in os.listdir(input_folder):
    if filename.endswith('.md'):
        ...
        output_file_name = filename.replace('.ja.md', '.tr.md')  # 替换后缀名
        ...
        for line in content[4:]:  # 从第五行开始翻译
            ...
            try:
                translated_line = GoogleTranslator(source='auto', target='turkish').translate(trimmed_line)
        ...
        for i, line in enumerate(final_content):
            if line.startswith('title ='):
                ...
                new_title = GoogleTranslator(source='auto', target='turkish').translate(original_title)  # 翻译标题
